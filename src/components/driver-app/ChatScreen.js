    ...
    <div className={`min-h-screen flex flex-col font-['Barlow_Condensed'] ${bg}`}>
      ...
      <div className={`px-4 py-4 flex items-center gap-3 border-b ${border}`}>
        ...
        <div className="flex-1">
          <h1 className={`text-xl font-bold tracking-wider ${textCls}`}>DISPATCHER CHAT</h1>
        ...
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-green-500 text-sm tracking-wider">LIVE</span>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto px-4 py-4 space-y-4">
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-12 px-4">
            ...
            <p className={`text-base text-center ${sub}`}>{fetchError}</p>
            <p className={`text-sm mt-1 text-center ${sub}`}>Chat endpoint may not be available yet. Contact your dispatcher by phone.</p>
          </div>
        ) : messages.length === 0 ? (
          <div className="text-center py-12">
            ...
            <p className={`text-base ${sub}`}>No messages yet.</p>
            <p className={`text-sm mt-1 ${sub}`}>Send a message to your dispatcher.</p>
          </div>
        ) : (
          messages.map((msg) => {
            ...
            return (
              <div key={msg.id} className={`flex flex-col ${isMe ? 'items-end' : 'items-start'}`}>
                {!isMe && (
                  <p className={`text-sm mb-1 ml-1 ${sub}`}>{msg.sender_name || 'Dispatcher'}</p>
                )}
                ...
                <p className={`text-sm mt-1 ${sub}`}>{formatTime(msg.sent_at)}</p>
              </div>
            );
          })
        )}
